...
export default function NavBar() {
  ...
  const { login, setLogin } = useContext(AppContext) as any;
          ...
          <>
            <div className="px-4 py-2 flex items-center cursor-pointer">
              ...
              <div className="relative w-14">
                <div onClick={toogleButtons}>
                  {localStorage.getItem("role") === "normal" ? "User" : "Admin"}
                </div>
                <div
                  className={`absolute z-40 w-32 py-2 bg-white shadow-lg px-2 rounded-lg ${
                    !show ? "hidden" : "block"
                  }`}
                >
                  <div
                    className={`py-2 border-b-2 cursor-pointer border-gray-700 hover:text-orange-500 ${
                      isAdmin ? "hidden" : "block"
                    }`}
                    onClick={toogleButtons}
                  >
                  ...
                  {isAdmin ? (
                    <>
                      <div
                        ...
                        onClick={toogleButtons}
                      >
                        <Link href="/AdminDashboard">All Users</Link>
                      </div>
                      <div
                        className="py-2 border-b-2 cursor-pointer border-gray-700 hover:text-orange-500"
                        onClick={toogleButtons}
                      >
                        <Link href="/AdminOrdersDashBoard">All Orders</Link>
                      </div>
                      <div
                        className="py-2 border-b-2 cursor-pointer border-gray-700 hover:text-orange-500"
                        onClick={toogleButtons}
                      >
                        <Link href="/AdminProductDashBoard">All Products</Link>
                      </div>
                    </>
                  ) : (
                    ""
            ...
            <div className="px-4 py-2 hover:text-orange-500 relative">
              <Link href="/Cart">
                ...
                <div className="absolute top-0 right-1 text-s font-bold bg-black w-5 h-5 text-orange-500 rounded-full text-center">
                  <p className="relative bottom-1">{cartItems?.length}</p>
                </div>
              </Link>
            </div>
          </>
      ...
      {isOpen && (
        <div className="lg:hidden">
          <div className="px-2 pt-2 pb-4">
            ...
            <div className="px-4 py-2 hover:text-orange-500">About</div>
            <div className="px-4 py-2 hover:text-orange-500">Contact</div>
            <div className="px-4 py-2 hover:text-orange-500"><Link href="/Orders">My Orders</Link></div>
