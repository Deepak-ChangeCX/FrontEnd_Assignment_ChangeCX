...
const OrderSummaryPage = () => {
  ...
  useEffect(() => {
    setId(router.query.order);
  }, [router.query.order]);

  useEffect(() => {
    if (id) {
      axios
        .get(`${URL}/api/v1/orders/id/${id}`, {
          headers: { authorization: localStorage.getItem("token") },
        })
        .then((res) => {
          console.log(res.data.orders);
          setOrderData(res.data.orders);
          const dateObj = new Date(res.data.orders.createdAt);
          setDateTime({
      ...
      <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        <div className="flex justify-start item-start space-y-2 flex-col">
          <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">
            Order #{orderData?._id?.split("").splice(0, 5).join("")}
          </h1>
        ...
        <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
          <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
            <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
              ...
              {orderData?.products?.map((items:any,idx:any)=>(
                <>
                ...
                <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                  ...
                  <div className="flex justify-between space-x-8 items-start w-full">
                    <p className="text-base dark:text-white xl:text-lg leading-6">
                      ${Math.ceil(items.price - (items.price * items.discountPercent) / 100)}.00{" "}
                      <span className="text-red-300 line-through"> ${items.price}.00</span>
                    </p>
                ...
                <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                  <div className="flex justify-between w-full">
                    ...
                    <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                      ${Math.ceil(orderData?.totalPrice + (5 * orderData?.totalPrice /100))}
                    </p>
                  </div>
                ...
                <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                  <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                    ...
                    <p className="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">
                      ...
                      <br></br>
                      {orderData?.shippingAddress?.admin_area_1}
                      <br></br>
                      {orderData?.shippingAddress?.admin_area_2}
                      <br></br>
